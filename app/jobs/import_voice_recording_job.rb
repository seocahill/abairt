class ImportVoiceRecordingJob < ApplicationJob
  ...
  def perform(voice_recording_id, url, importer_class, title: nil)
    ...
    # Skip if already completed successfully and has dictionary entries
    if voice_recording.import_status == 'completed' && voice_recording.dictionary_entries.exists?
      Rails.logger.info "Voice recording import skipped - already completed for ID: #{voice_recording.id}"
      return
    end
